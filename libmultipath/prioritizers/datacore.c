		   ...
		   unsigned int timeout_ms)
{
	...
	if (sscanf(args, "timeout=%i preferredsds=%254s", (int *)&timeout_ms,
		   preferredsds) == 2) {
	} else if (sscanf(args, "preferredsds=%254s timeout=%i", preferredsds,
			  (int *)&timeout_ms) == 2) {
	} else if (sscanf(args, "preferredsds=%254s", preferredsds) == 1) {
	} else {
